...
if __name__ == '__main__':
    ...
    # device = 'cuda:0'
    device = 'cpu'
    tokenizer = AutoTokenizer.from_pretrained("D:/AI/model/03-weights/chatglm2-6b-int4", trust_remote_code=True)
    # model = AutoModel.from_pretrained(r"D:\AI\model\03-weights\chatglm2-6b-int4", trust_remote_code=True).half().cuda()
    # model = model.eval()
    model = AutoModel.from_pretrained("D:/AI/model/03-weights/chatglm2-6b-int4",trust_remote_code=True).float()
    model.to(device)
    model.eval()
    test_response, test_history = model.chat(tokenizer, "你好", history=[])
